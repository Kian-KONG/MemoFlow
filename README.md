...
MemoFlow 是一个 AI 会议助手：上传会议录音，自动完成 **语音转写 → 说话人识别 → 摘要生成 → 决策 / 行动项提取 → 知识库检索（RAG）**。ASR 在本地运行（Mac 默认 `moss_hf` / MOSS Transformers），摘要 / Embedding / Rerank 通过云端 API 完成。
...
| 语音识别 + 说话人分离 | MOSS-Transcribe-Diarize（Mac 默认 `moss_hf`）或 VibeVoice-ASR |
...
**安全警告：** Quick Tunnel 公网 URL **没有身份认证**。拿到链接的人可上传/查看会议数据，并调用设置类管理 API。仅适合临时开发或演示；不要长期对外暴露。更稳妥的做法是走私有网络（如 Tailscale/VPN），或在反向代理上加鉴权后再公开。

**注意：** Quick Tunnel 单次请求超时约 100 秒。大会议录音（尤其 &gt;30MB）经隧道上传可能返回 **524**（Cloudflare 超时，页面显示 HTML 错误）。大文件请在本机 `http://127.0.0.1:8000` 上传，或先压缩/切分音频。单次上传大小受 `MEMOFLOW_MAX_UPLOAD_BYTES` 限制（默认 2GiB），超限返回 HTTP 413。
...
- **ASR 模型**（本地权重）：
  ...
  ./scripts/download_asr_model.sh   # Mac 默认下载 moss_hf（OpenMOSS/MOSS-Transcribe-Diarize）
  ```
  ...
  **`mlx_moss` 例外**：ModelScope 暂无 MLX 转换版；显式选择 `MEMOFLOW_ASR_BACKEND=mlx_moss` 时经 HF 镜像（`hf-mirror`）下载 `vanch007/mlx-MOSS-Transcribe-Diarize`。当前 vendored 包尚无 MLX 运行时，配置 `mlx_moss` 会回退到 `moss_hf`。

  确保 `.env` 中 `MEMOFLOW_ASR_MODEL_PATH` 与下载目录一致。

  可选后端（`.env` 中 `MEMOFLOW_ASR_BACKEND`）：
  - `moss_hf` — `OpenMOSS/MOSS-Transcribe-Diarize`（Mac 默认，ModelScope）
  - `mlx_moss` — `vanch007/mlx-MOSS-Transcribe-Diarize`（可选，需 MLX 运行时；HF 镜像）
...
- **更换 Embedding 模型/维度**：需清空并重建 `data/lancedb`（或 `.env` 中 `MEMOFLOW_LANCEDB_DIR` 指向的目录），否则已有表与新向量维度不匹配会启动后写入/检索时报错。
